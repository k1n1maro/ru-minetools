...
# Импорт JAR переводчика
try:
    # Пробуем импортировать из текущей папки (для EXE)
    if getattr(sys, 'frozen', False):
        # В режиме EXE файлы находятся в корне
    ...
    JAR_TRANSLATOR_AVAILABLE = True
    print("✅ JAR переводчик загружен")
except ImportError as e:
    JAR_TRANSLATOR_AVAILABLE = False
    print(f"⚠️ JAR переводчик недоступен: {e}")
    # Создаем заглушки
    def translate_jar(*args, **kwargs):
        raise ImportError("JAR переводчик не загружен")
    def load_translation_cache():
        return {}
    def save_translation_cache():
        pass
    TRANSLATION_CACHE = {}
    def has_russian_lang(*args):
        return False
    def has_russian_patchouli(*args):
        return False
    def analyze_jar_files(*args, **kwargs):
        return []
...
try:
    ...
    if getattr(sys, 'frozen', False):
        ...
        from update_config import UPDATE_SETTINGS, CURRENT_VERSION
    ...
    print("✅ Система обновлений загружена")
except ImportError as e:
    UPDATER_AVAILABLE = False
    UPDATE_SETTINGS = {"auto_check": True}  # Значение по умолчанию
    CURRENT_VERSION = "1.0.0"  # Fallback версия
    print(f"⚠️ Система обновлений недоступна: {e}")
    logger.debug(f"Система обновлений недоступна: {e}")
...
class WelcomeBackOverlay(QWidget):
    ...
    def create_header(self, layout):
        ...
        if os.path.exists(logo_path):
            ...
            # Пробуем загрузить наше лого logow.PNG
            logow_path = get_asset_path("logow.PNG")
            if os.path.exists(str(logow_path)):
                pixmap = QPixmap(str(logow_path))
                scaled_pixmap = pixmap.scaled(140, 140, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)
                logo_label.setPixmap(scaled_pixmap)
            else:
                logo_label.setText("RU-MINETOOLS")
                logo_label.setStyleSheet("font-size: 20px; font-weight: bold; color: #E06BFF;")
...
class TelegramAuthOverlay(QWidget):
        ...
        try:
            ...
            if os.path.exists(config_path):
                with open(config_path, 'r', encoding='utf-8') as f:
                    ...
                    self.API_URL = config.get("API_URL", "https://d5dq2g7pcv53nkqcsp1p.svoluuab.apigw.yandexcloud.net")
                    self.BOT_URL = config.get("BOT_URL", "https://t.me/ru_minetools_auth_bot")
                    self.CHANNEL_URL = config.get("CHANNEL_URL", "https://t.me/ruquestbook")
    ...
    def create_header(self, layout):
        ...
        logo_label = QLabel()
        logo_path = str(get_asset_path("logow.PNG"))  # Используем наше лого
        if os.path.exists(logo_path):
            pixmap = QPixmap(logo_path)
            ...
            # Fallback к тексту вместо геймпада
            logo_label.setText("RU-MINETOOLS")
            logo_label.setStyleSheet("font-size: 20px; font-weight: bold; color: #E06BFF;")
        logo_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
    ...
    def _check_bot_availability(self):
        ...
        try:
            ...
            yandex_url = f"{self.API_URL}/check/test123"
    ...
    def _proceed_with_bot_auth(self):
        ...
        bot_url = self.BOT_URL or "https://t.me/ru_minetools_auth_bot"
    ...
    def _verify_auth_code(self, auth_code):
        ...
        try:
            ...
            api_url = f"{self.API_URL}/check/{auth_code}"
...
class Sidebar(QFrame):
    ...
    def create_header(self):
        ...
        self.title_label = QLabel(f"RU-MINETOOLS v{CURRENT_VERSION}")
    ...
    def open_telegram_community(self):
        ...
        try:
            channel_url = getattr(self, 'CHANNEL_URL', None) or "https://t.me/ruquestbook"
...
📁 Выберите JAR файл(ы) → 🚀 Нажмите "Начать перевод"
        ...
        self.threads_value = "4"  # Текущее значение
        threads_container = QWidget()
        threads_container.setFixedSize(80, 32)
    ...
    def create_settings_page(self):
        ...
        # Новые возможности в версии 1.1.0 и 1.2.0
        try:
            from update_config import CURRENT_VERSION
            if CURRENT_VERSION == "1.1.0":
                new_features_label = QLabel("🆕 Новое в версии 1.1.0: Улучшенное исключение названий модов")
                new_features_label.setStyleSheet("""
                    QLabel {
                        color: #10b981;
                        font-size: 13px;
                        font-weight: 600;
                ...
                """)
                info_layout.addWidget(new_features_label)
            elif CURRENT_VERSION == "1.2.0":
                new_features_label = QLabel("🆕 Новое в версии 1.2.0: Исправлена система обновлений")
                new_features_label.setStyleSheet("""
                    QLabel {
                        color: #3b82f6;
                        font-size: 13px;
...
📁 Выберите JAR файл(ы) → 🚀 Нажмите "Начать перевод"
    ...
    def on_jar_progress_update(self, progress, message):
        """Обновление прогресса перевода JAR модов"""
        if progress >= 0:
...
class ModernGUIInterface(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.setWindowTitle(f"RU-MINETOOLS v{CURRENT_VERSION}")
...
Способы поддержки:""")
    ...
    def show_beta_warning_dialog(self):
        ...
        # Проверяем версию для показа соответствующего сообщения
        try:
            from update_config import CURRENT_VERSION
            if CURRENT_VERSION == "1.1.0":
                version_info = """<p style="font-size: 14px; color: #10b981;"><b>🆕 Версия 1.1.0:</b> Улучшено исключение названий модов из перевода. Теперь названия популярных модов (Simple Hats, Thermal Expansion и др.) остаются на английском языке!</p>"""
            elif CURRENT_VERSION == "1.2.0":
                version_info = """<p style="font-size: 14px; color: #3b82f6;"><b>🆕 Версия 1.2.0:</b> Исправлена система обновлений! Теперь старые версии корректно удаляются, а зеленое окно консоли ждет подтверждения пользователя.</p>"""
            else:
                version_info = ""
        except ImportError:
            version_info = ""
        
        intro_text = QLabel(f"""<div style="text-align: center;">
...
🚀 Нажмите "Начать перевод" для запуска
    ...
    def on_jar_progress_update(self, progress, message):
        """Обновление прогресса перевода JAR"""
        self.jar_progress.setValue(progress)
        self.jar_progress.setText(message)
...
class SimpleJarTranslationWorker(QThread):
    ...
    def __init__(self, input_path, output_path, replace_original=False, selected_files=None, analysis=None, threads_count=4):
        super().__init__()
        ...
        self.progress_lock = threading.Lock()  # Блокировка для синхронизации прогресса
        self.last_progress = {}  # {jar_index: last_progress_percent} для отслеживания изменений
    
    def calculate_total_progress(self, total_jars):
        """Вычисляет общий прогресс на основе завершенных и активных модов"""
        with self.progress_lock:
            # Считаем завершенные моды (100% каждый)
            ...
            active_progress = 0
            if self.jar_progress:
                max_active_progress = max(self.jar_progress.values())
                # Добавляем прогресс одного активного мода
                active_progress = (max_active_progress / 100) * (100 / total_jars)
            
            total_progress = min(completed_progress + active_progress, 100)
    ...
    def _process_jar_threaded(self, jar_file, jar_index, total_jars):
        ...
        try:
            ...
            # Отмечаем мод как завершенный даже при ошибке для правильного расчета прогресса
            with self.progress_lock:
                self.completed_jars.add(jar_index)
                if jar_index in self.jar_progress:
                    del self.jar_progress[jar_index]
                # Очищаем последний прогресс
                if jar_index in self.last_progress:
                    del self.last_progress[jar_index]
    ...
    def _process_jar_core(self, jar_file, jar_index, total_jars, output_capture, old_stdout):
        ...
        def file_progress_callback(progress, current, total):
            ...
            # Обновляем каждый процент для плавного отображения прогресса
            should_update = (
                int(mod_progress) != int(self.last_progress.get(jar_index, -1)) or  # Каждый новый процент
                mod_progress == 100 or     # Обязательно при 100%
                current == 1               # Первая строка
            )
            
            # Сохраняем последний прогресс для этого мода
            self.last_progress[jar_index] = mod_progress
            
            if should_update:
                # Обновляем прогресс для этого мода
                with self.progress_lock:
                    self.jar_progress[jar_index] = mod_progress
                
                # Вычисляем общий прогресс
                total_progress = self.calculate_total_progress(total_jars)
                
                # Обновляем главный прогресс-бар только если он увеличился
                self.progress_updated.emit(total_progress, f"Мод {jar_index+1}/{total_jars}: {jar_name} - {mod_progress}%")
                ...
                with ThreadPoolExecutor(max_workers=self.threads_count) as executor:
                    ...
                    for future in as_completed(future_to_jar):
                        ...
                        try:
                            ...
                            # НЕ обновляем прогресс здесь - он уже обновляется в callback
                            # Прогресс теперь рассчитывается правильно в calculate_total_progress
