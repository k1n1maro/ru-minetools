...
class ModernUpdateWorker(QThread):
    ...
    def create_update_script(self, new_exe_path, current_exe_path, version_info=None):
        ...
        try:
            ...
            # Создаем улучшенный batch скрипт с правильной обработкой ошибок
            batch_content = f"""@echo off
chcp 65001 >nul
title Обновление RU-MINETOOLS до v{version}
color 0A
...
echo    (Программа должна закрыться автоматически)
echo.

:wait_loop
tasklist /fi "imagename eq {current_exe.name}" 2>nul | find /i "{current_exe.name}" >nul
if not errorlevel 1 (
    echo    ⏳ Программа еще работает, ждем...
    timeout /t 2 /nobreak >nul
    goto wait_loop
)

REM Дополнительная пауза для полного закрытия
timeout /t 3 /nobreak >nul

echo ✅ Программа закрыта успешно
echo.

if not exist "{permanent_new_exe}" (
    echo ❌ ОШИБКА: Новый файл не найден!
    echo    Файл: {permanent_new_exe}
    goto error_exit
)
...
echo 🗑️ Поиск и удаление старых версий программы...

REM Удаляем все старые версии ru-minetools (включая с номерами в скобках)
for %%f in ("{current_exe.parent}\\ru-minetools*.exe") do (
    if not "%%f"=="{permanent_new_exe}" (
        if exist "%%f" (
            echo    Удаление: %%~nxf
            del "%%f" >nul 2>&1
            if not exist "%%f" (
                ...
                echo    ❌ Не удалось удалить: %%~nxf
                REM Пытаемся удалить принудительно
                attrib -r "%%f" >nul 2>&1
                del /f "%%f" >nul 2>&1
                if not exist "%%f" (
                    echo    ✅ Принудительно удален: %%~nxf
                ) else (
                    echo    ❌ Файл заблокирован: %%~nxf
                )
            )
        )
    )
...
REM Удаляем backup файлы
for %%f in ("{current_exe.parent}\\*.backup") do (
    if exist "%%f" (
        echo    Удаление backup: %%~nxf
        del "%%f" >nul 2>&1
    )
)
...
echo ✅ Новая версия готова: {permanent_new_exe.name}
echo 📁 Расположение: {current_exe.parent}
echo.

echo ⚠️ ВНИМАНИЕ: НЕ ЗАКРЫВАЙТЕ ЭТО ОКНО!
echo    Сейчас запустится новая версия программы
echo.
echo 🚀 Запуск новой версии через 3 секунды...
timeout /t 3 /nobreak >nul

start "" "{permanent_new_exe}"

if errorlevel 1 (
    echo ❌ Ошибка запуска новой версии
    goto error_exit
)

REM Ждем запуска новой версии
...
echo 📄 Новый файл: {permanent_new_exe.name}
echo 🗑️ Старые версии удалены
echo.
echo 🎉 Новая версия RU-MINETOOLS v{version} запущена!
echo.
echo ⚠️ ВАЖНО: НЕ ЗАКРЫВАЙТЕ ЭТО ОКНО САМИ!
echo    Это окно должно оставаться открытым
echo    пока вы не убедитесь что новая версия работает
echo.
echo 📋 Проверьте:
echo    1. Новая версия программы запустилась
echo    2. Авторизация работает корректно  
echo    3. Все функции доступны
echo.
echo ✅ Если все работает - нажмите любую клавишу для закрытия
pause >nul
echo.
echo 👋 Обновление завершено успешно!
echo    Окно закрывается через 3 секунды...
timeout /t 3 /nobreak >nul
exit
...
REM Пытаемся найти любую версию программы для запуска
echo 🔍 Поиск доступных версий программы...
for %%f in ("{current_exe.parent}\\ru-minetools*.exe") do (
    if exist "%%f" (
        echo 🔄 Найдена версия: %%~nxf
        echo    Запуск...
        start "" "%%f"
        echo ✅ Запущена версия: %%~nxf
        goto end_error
    )
)

echo ❌ Не найдено ни одной версии программы!
echo    Скачайте программу заново с GitHub
